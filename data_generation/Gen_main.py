...
#VERSON NOTE: only HMO with predefined energy function will work
...
if __name__ == "__main__":
    ...
    if g != None:
        for i in range(int(args.n_samps/args.n_per)):
            ...
            g.export_samps("samples_"+str(args.res)+"_["+str(args.n_per*(i+1))+" of "+str(args.n_samps)+"]")
            g.clear_samps()
        if args.n_samps%args.n_per != 0:
            g.gen_samps(args.n_samps%args.n_per)
            g.export_samps("samples_"+str(args.res)+"_["+str(args.n_samps)+" of "+str(args.n_samps)+"]")
            g.clear_samps()